...
void View::paintEvent(QPaintEvent* e)
{
    ...
    QSize dSize((static_cast<int>(m_roi.width() * m_scale) + 3) / 4 * 4, static_cast<int>(m_roi.height() * m_scale));
#if 0
    ...
    m_painter.drawImage((m_viewSize.width() - src_roi.width()) / 2, (m_viewSize.height() - src_roi.height()) / 2, src_roi, 0, 0, src_roi.width(), src_roi.height());
...
void View::updateParm()
{
    ...
    m_roi.setWidth(qMin(static_cast<int>(m_viewSize.width() / m_scale), m_image->width()));
    m_roi.setHeight(qMin(static_cast<int>(m_viewSize.height() / m_scale), m_image->height()));

    m_roi.setX(static_cast<int>(RANGE(0.0, m_roi.x() + m_mouse_offset.x(), m_image->width() - m_roi.width())));
    m_roi.setY(static_cast<int>(RANGE(0.0, m_roi.y() + m_mouse_offset.y(), m_image->height() - m_roi.height())));
